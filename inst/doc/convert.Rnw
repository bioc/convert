...
%\VignetteDepends{convert, Biobase, marrayClasses, limma}
...
the \texttt{convert} Package Version 1.1}
\author{Gordon Smyth and Robert Wettenhall}
\date{March 12, 2004}
\maketitle

The convert package provides the ability to convert between microarray data formats (object classes) defined in the packages Biobase, limma and marrayClasses.
...
\item
\texttt{marrayNorm} (marrayClasses).
	Stores red and green channel foreground and background intensities and associate information for a batch of spotted microarrays.
	Analogous to \texttt{MAList}.
\item
\texttt{exprSet} (Biobase).
	...
	Commonly used for single channel data such as Affymetrix or for pre-processed two-color data in the form of log-ratios ready for cluster analysis or classification.
\end{itemize}
The \texttt{convert} package provides conversion to and from \texttt{RGList} and \texttt{marrayRAw}, to and from \texttt{MAList} and \texttt{MarrayNorm}, from \texttt{RGList} to \texttt{exprSet} and from \texttt{MAList} to \texttt{exprSet}.
